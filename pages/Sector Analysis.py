...
def get_market_info(market):
    market_info = (Query().select('country','name','exchange','market','sector','close', 'vwap','volume','relative_volume_10d_calc', 'market_cap_basic').
    ...
    if market == 'egypt':
        ...
        info = pd.concat([infot[['name','exchange','close', 'vwap','volume','relative_volume_10d_calc','market_cap_basic']], infor.sector], axis=1, join='inner')
    else:
        info = market_info
    return info
...
st.plotly_chart(fig)
if market != "america":
    st.write(info[info.sector==sector][['name','close','vwap','volume','relative_volume_10d_calc','market_cap_basic']])
