...
if market == 'america':
    ...
    stocks["Last Sale"]=stocks["Last Sale"].str.replace(",","").astype(float)
    stocks=stocks[(stocks['Sector']==sector_name) & (stocks['Volume']>500000) & (stocks['Last Sale']>10.0)]['Symbol']
...
df_returns= np.diff(np.log(df))[1:]
...
fig = go.Figure()
fig.add_trace(go.Scatter(y= df.loc[params.index,'INDEX'] , x=params.index, name='Index', mode='lines'))
